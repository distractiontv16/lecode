      ...
      <View style={styles.cardContent}>
        <View style={styles.levelHeader}>
          ...
          <Text style={styles.levelText}>
            {language === 'fr' ? 'Niveau' : 'Level'} {difficulty === 'facile' ? (language === 'fr' ? 'Facile' : 'Easy') : difficulty === 'moyen' ? (language === 'fr' ? 'Moyen' : 'Medium') : (language === 'fr' ? 'Difficile' : 'Hard')}
          </Text>
        ...
        {isLocked ? (
          <View style={styles.lockedContainer}>
            <Text style={styles.lockedText}>{language === 'fr' ? 'Niveau verrouillé' : 'Level locked'}</Text>
            <Text style={styles.unlockText}>
              {difficulty === 'moyen' 
                ? language === 'fr' ? 'Terminez toutes les catégories du niveau Facile' : 'Complete all categories in Easy level'
                : language === 'fr' ? 'Terminez toutes les catégories du niveau Moyen' : 'Complete all categories in Medium level'}
            </Text>
          </View>
        ) : (
          <View style={styles.progressContainer}>
            <View style={styles.progressInfo}>
              <Text style={styles.progressText}>{progress}% {language === 'fr' ? 'complété' : 'completed'}</Text>
              {progress === 100 && (
                <View style={styles.completedBadge}>
                  <Text style={styles.completedText}>{language === 'fr' ? 'Terminé !' : 'Completed!'}</Text>
                </View>
              )}
            </View>
            ...
            <Text style={styles.categoriesText}>
              {progress < 100 
                ? language === 'fr' 
                  ...
                  : `${Math.round(progress/20)} categories out of 5 completed`
                : language === 'fr' 
...
export default function QuizScreen() {
  ...
  const [noHeartsWarning, setNoHeartsWarning] = useState<boolean>(false);
  ...
  const getLevels = () => {
    ...
    return defaultLevels.map(level => {
      const difficultyProgress = userProgress.difficulties.find(
        (d: DifficultyProgress) => d.difficulty === level.difficulty
      );
  ...
  return (
    <SharedTransition transitionKey="quiz-screen">
      <View style={styles.container}>
        {/* Header fixe en haut */}
        <Header xp={userProgress?.totalXP || contextXP || 0} />
        ...
        >
          ...
          <View style={styles.content}>
            ...
            {/* Message d'avertissement si pas de cœurs */}
            {noHeartsWarning && (
              <View style={styles.noHeartsContainer}>
                <Ionicons name="heart-dislike" size={24} color="#FF5252" />
                <Text style={styles.noHeartsTitle}>Tu n'as plus de vies !</Text>
                <Text style={styles.noHeartsText}>
                  Tu dois attendre 1 heure pour en récupérer une et continuer.
                </Text>
                {regenerationTime && (
